...
def runtime_args(model_dir: Path, ram_gb: int, vram_gb: float, context: int, gpu: str) -> list[str]:
    ...
    if gpu == "none" and vram_gb:
        raise click.BadParameter(
            "A VRAM budget requires explicit --gpu device IDs", param_hint="--vram-gb"
        )
    if gpu != "none" and not vram_gb:
        raise click.BadParameter(
            "Set an explicit --vram-gb budget for GPU use", param_hint="--vram-gb"
        )
...
def run_inspection(
    command: list[str], args: list[str], operation: str = "plan"
) -> tuple[dict, int]:
    try:
        result = subprocess.run(
            command + [operation, *args, "--json"],
            capture_output=True,
            text=True,
            timeout=30,
            check=False,
        ...
        raise click.ClickException(
            "Colibri planning exceeded 30 seconds; no inference server was started."
        ) from error
    ...
    if result.returncode and operation == "plan":
        raise click.ClickException(
            f"Colibri plan failed ({result.returncode}): {(result.stderr or result.stdout)[-2000:]}"
        )
    try:
        plan = json.loads(result.stdout)
        if not isinstance(plan, dict) or (
            operation == "plan" and not isinstance(plan.get("model"), dict)
        ):
            raise ValueError("Missing model resource information")
        if operation == "doctor" and (
        ...
        raise click.ClickException(
            "Colibri returned an incompatible plan; check its version before serving."
        ) from error
    ...
    for decorator in reversed(
        [
            click.option(
                ...
                help="Existing coli Python launcher or native executable.",
            ),
            click.option(
                "--model-dir",
                required=True,
            ...
            click.option(
                ...
                help="Explicit Colibri RAM budget; leave room for the OS.",
            ),
            click.option(
                "--vram-gb", default=0.0, type=click.FloatRange(0, 65536), show_default=True
            ),
            click.option(
                "--context", default=8192, type=click.IntRange(1024, 131072), show_default=True
            ),
            click.option(
                "--gpu",
                default="none",
                show_default=True,
                help="none or explicit device IDs (e.g. 0,1).",
            ),
        ]
    ):
...
def plan(launcher, model_dir, ram_gb, vram_gb, context, gpu):
    ...
    result, _ = run_inspection(
        launcher_command(launcher), runtime_args(model_dir, ram_gb, vram_gb, context, gpu)
    )
    click.echo(
        json.dumps(
            {
...
@click.option(
    "--dry-run", is_flag=True, help="Validate the plan and print argv without loading weights."
)
def serve(launcher, model_dir, ram_gb, vram_gb, context, gpu, port, max_tokens, model_id, dry_run):
    ...
    if max_tokens >= context:
        raise click.BadParameter(
            "Output budget must be smaller than --context", param_hint="--max-tokens"
        )
    if not re.fullmatch(r"[A-Za-z0-9._/-]{1,128}", model_id):
        raise click.BadParameter(
            "Use 1–128 letters, digits, dots, underscores, slashes or hyphens",
            param_hint="--model-id",
        )
    ...
    descriptor = next(
        (
            entry.get("details", {}).get("descriptor", {})
    ...
    if doctor_exit:
        failures = [
            item.get("summary", item.get("id", "check"))
...
def check(url):
    ...
    try:
        with httpx.Client(timeout=5) as client:
            ...
            models = [
                item["id"] for item in response.json()["data"] if isinstance(item.get("id"), str)
            ]
        if not models:
            raise ValueError("No model IDs returned")
    except (httpx.HTTPError, ValueError, KeyError, TypeError, AttributeError) as error:
        raise click.ClickException(
            f"Colibri model discovery failed: {type(error).__name__}"
        ) from error
    click.echo(
        json.dumps(
            {
                "url": base,
                "models": models,
                "tool_support": "not advertised by upstream discovery",
                "inference_tested": False,
            },
            indent=2,
        )

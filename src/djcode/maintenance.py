...
def run_checks() -> dict:
    ...
    try:
        ...
        checks.append(
            {"name": "Python syntax", "status": "passed", "detail": f"{len(files)} modules"}
        )
    ...
    try:
        ...
        checks.append(
            {
                "name": "Runtime registries",
                "status": "passed",
                "detail": f"{len(PROVIDERS)} providers, {len(names)} tools",
            }
        )
    ...
    return {
        "ok": ok,
        "summary": "Installation checks passed." if ok else "Installation checks failed.",
        "checks": checks,
    }
